...
const Settings_content: React.FC = () => {
  ...
  const handleRemoveListings = async () => {
    try {
      ...
      const response = await axios.post(
        "https://metashot-backend.azurewebsites.net/interview/delete",
        {
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      const response = await apiClient.post('/user/upload-pfp', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      // const response = await axios.post('http://localhost:8000/user/upload-pfp', formData, {
      //   headers: {
      //     'Content-Type': 'multipart/form-data',
      //   },
      // });
  ...
  return (
    <>
      <div
        className={`flex flex-col mt-8 rounded-xl text-xs shadow-md ${styles["container"]} text-black`}
      >
        <div className="w-3/4">
          <h2 className="mt-6 mb-8 ml-5 text-xl font-semibold">Settings</h2>

          {/* Profile Picture */}
          <div className={`flex flex-row items-center justify-start space-x-2 ${styles["inputBar"]}`}>
            <div className="flex items-center justify-end h-6 ml-5 font-bold w-36">
              Profile Picture
            </div>
            <div className="flex space-x-3 items-center justify-start h-6 ml-10 w-28">
              <input
                type="file"
              ...
              <label
                htmlFor="fileInput"
                className="bg-[#FFFFFF] px-3 pt-1 font-bold rounded-xl border-solid border-2 hover:shadow-lg w-32 h-10 mb-4 hover:cursor-pointer text-center text-[0.3] "
              >
                <p className="mx-auto my-auto">CHOOSE FILE</p>
              </label>
            ...
            </div>
              {
                ...
                className="bg-blue-500 text-white p-3 font-bold rounded-xl shadow-lg w-32 h-10 mb-4"
              >
                Upload
              </button>
                ):null
          ...
          <div className={`flex mt-5 mb-4 ${styles["inputBar"]}`}>
            ...
            <div
              className={`flex items-center justify-start h-6 ml-10 w-72 p-2 border rounded-md ${styles["box"]}`}
            >
              {/* <input type="text" className={`p-2 border rounded-md ${styles['box']}`}  /> */}
              {companyName}
            </div>
            ...
            <div className="flex items-center justify-start h-6 ml-10 w-72">
              <FontAwesomeIcon
                ...
                className={`text-2xl cursor-pointer ${
                  exportCandidateList ? "text-green-500" : ""
                } `}
                onClick={() => {
